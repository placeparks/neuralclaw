...
begin
  ...
  if not exists (
    select 1 from pg_enum e
    join pg_type t on t.oid = e.enumtypid
    where t.typname = 'provider_key' and e.enumlabel = 'chatgpt_token'
  ) then
    alter type provider_key add value 'chatgpt_token';
  end if;
  if not exists (
    select 1 from pg_enum e
    join pg_type t on t.oid = e.enumtypid
    where t.typname = 'provider_key' and e.enumlabel = 'claude_token'
  ) then
    alter type provider_key add value 'claude_token';
  end if;
  if not exists (
    select 1 from pg_enum e
    join pg_type t on t.oid = e.enumtypid
    where t.typname = 'provider_key' and e.enumlabel = 'chatgpt_session'
  ) then
    alter type provider_key add value 'chatgpt_session';
  end if;
  if not exists (
    select 1 from pg_enum e
    join pg_type t on t.oid = e.enumtypid
    where t.typname = 'provider_key' and e.enumlabel = 'claude_session'
  ) then
    alter type provider_key add value 'claude_session';
  end if;
end $$;
...
begin
  if exists (
    ...
    -- Backfill agents from legacy deployments table
    insert into public.agents (
      id, user_id, agent_name, plan, provider, provider_api_key_encrypted,
      model, region, status, railway_service_id, railway_deployment_id,
      provision_attempts, error_message, provisioning_started_at,
  ...
  if exists (
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'deployment_channels'
  ) then

    -- Backfill channels from legacy deployment_channels table
